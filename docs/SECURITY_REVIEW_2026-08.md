# 2026-08 콜드월렛 사건 기반 보안 검토

검토 기준일: 2026-08-12 (Asia/Seoul)

## 결론

2026년 8월 기준 최근 대형 콜드월렛 사건은 COLDCARD 펌웨어의 시드 생성 난수 결함으로 생성된 키가 예측 가능해지고, 실제 온체인 탈취로 이어진 사례다. 토리월렛은 하드웨어 콜드월렛이 아니라 모바일 소프트웨어 지갑이므로 펌웨어·물리 장치 공격면은 같지 않다. 다만 **시드 생성기가 비보안 난수로 조용히 폴백할 수 있는 위험**은 공통으로 적용 가능했다.

이번 변경으로 현재 토리월렛의 지갑 생성은 OS CSPRNG를 확인할 수 없거나 비정상 출력이 감지되면 실패하도록 바뀌었다. 따라서 확인된 COLDCARD 사건과 같은 종류의 조용한 예측 가능 난수 폴백 경로는 현재 코드에서 차단된다. 이 결론은 소스·단위 테스트·네이티브 빌드에 근거하며, 외부 전문 감사나 과거에 이미 생성된 모든 니모닉의 엔트로피를 소급 증명하는 것은 아니다.
...
2026-08-12 재검색 기준 Coinkite의 공식 경고는 2026-08-01 갱신본이 최신이었고, 신뢰할 수 있는 매체에서 확인된 가장 최근의 구체적 피해 추정은 2026-08-04 보도였다. 이후의 더 큰 수치는 1차 자료나 독립 검증이 부족해 결론에 사용하지 않았다.
...
| 사건의 공격면 | 변경 전 토리월렛 | 이번 조치 | 현재 판단 |
| --- | --- | --- | --- |
| 비보안 RNG 폴백 | 네이티브에서는 iOS `SecRandomCopyBytes`/Android `SecureRandom`을 사용했지만, 구형 Chrome 원격 디버거 폴리필이 `Math.random`으로 폴백할 수 있었음 | 원격 디버거 감지, CSPRNG 부재·오류, 단일 반복 바이트·반복 절반·직전 출력 재사용 시 지갑 생성을 즉시 중단 | 동일 유형의 조용한 폴백 차단 |
